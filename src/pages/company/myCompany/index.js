...
import CompanyCard from "../../../components/screen/company/companycard";
...
const MyCompanyIndex2 = ({ initProps, dataProfile, sidemenu }) => {
  ...
  const handleDeleteBank = () => {
    ...
    fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/deleteMainBank?id=${hapusbankdata.id}`,
      {
        method: "DELETE",
        headers: {
  ...
  const handleEditBank = () => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/updateMainBank`, {
      method: "PUT",
      headers: {
        Authorization: JSON.parse(initProps),
