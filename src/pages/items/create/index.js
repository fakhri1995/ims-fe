...
const ItemCreate = ({ initProps, sidemenu, dataProfile }) => {
  ...
  const searchPart = (doc, partid) => {
    for (var i = 0; i < doc.length; i++) {
      if (doc[i].model_id === partid) {
        const mandatoryfieldspec = doc[i].inventory_values
          .filter((docfil) => docfil.required === true)
          .map((docmap) => {
        ...
        if (
          doc[i].inventory_name === "" ||
          doc[i].mig_id === "" ||
          doc[i].status_condition === "" ||
          doc[i].status_usage === "" ||
          doc[i].serial_number === "" ||
          mandatoryfieldspec.includes("") ||
          mandatoryfieldspec.includes("-")
        ) {
          return true;
        } else {
    ...
    for (var i = 0; i < doq.length; i++) {
      if (doq[i].model_id === Number(partid)) {
        ...
        if (doq[i].inventory_parts) {
          arr.push({
            ...doq[i],
            inventory_parts: changeDataPart(
              doq[i].inventory_parts,
              partid,
              attr,
              value
    ...
    for (var i = 0; i < doq.length; i++) {
      if (doq[i].model_id === Number(partid)) {
        ...
        if (doq[i].inventory_parts) {
          arr.push({
            ...doq[i],
            inventory_parts: changeInventoryValuesPart(
              doq[i].inventory_parts,
              partid,
              idcolumn,
              value
    ...
    for (var i = 0; i < doq.length; i++) {
      if (doq[i].model_id === Number(partid)) {
        ...
        if (doq[i].inventory_parts) {
          arr.push({
            ...doq[i],
            inventory_parts: changeInventoryValuesDropdownPart(
              doq[i].inventory_parts,
              partid,
              idcolumn,
              value
  ...
  const changeInventoryValuesCheckboxPart = (
    doq,
    partid,
    idcolumn,
    checked,
    ...
    for (var i = 0; i < doq.length; i++) {
      if (doq[i].model_id === Number(partid)) {
        ...
        if (checked === true) {
          ...
          var idxtoremove =
            doq[i].inventory_values[idxfield].value.default.indexOf(idx);
          doq[i].inventory_values[idxfield].value.default.splice(
            idxtoremove,
            1
          );
          doq[i].inventory_values[idxfield].default.default.splice(
            idxtoremove,
            1
          );
        }
        ...
        if (doq[i].inventory_parts) {
          arr.push({
            ...doq[i],
            inventory_parts: changeInventoryValuesCheckboxPart(
              doq[i].inventory_parts,
              partid,
              idcolumn,
              checked,
        ...
        <Collapse>
          {
            <Panel
              id={`panel${idx}`}
              key={idx}
              header={<strong>{doc.model_name}</strong>}
              extra={
                <div className="flex">
                  <Checkbox
                  ...
                  ></Checkbox>
                  <p className="mb-0">
                    {doc.enable_part ? "Item Ada" : "Item Tidak Ada"}
                  </p>
                </div>
              }
            >
                          ...
                          {docvalue.data_type === "date" && (
                            <DatePicker
                              ...
                              onChange={(date, datestring) => {
                                var temp = newdata.inventory_parts;
                                const selectedpart = changeInventoryValuesPart(
                                  temp,
                                  doc.model_id,
                                  docvalue.id,
                                  datestring
              ...
              {doc.inventory_parts.length > 0 &&
                renderChildPartModel(doc.inventory_parts)}
            </Panel>
          }
        </Collapse>
      </Timeline.Item>
      ...
      for (var i = 0; i < item.length; i++) {
        ...
        if (item[i].enable_part) {
          temp11.push({
            ...
            inventory_parts:
              item[i].inventory_parts.length > 0
                ? recursivePartItem(item[i].inventory_parts)
                : [],
          });
        }
      }
  ...
  useEffect(() => {
    if (emptyfieldparttrigger !== 0) {
      if (emptyfieldpartmodel.enable_part === true) {
        const bool = searchPart(
          newdata.inventory_parts,
          emptyfieldpartmodel.model_id
        );
        if (bool) {
          if (emptyfieldpart.indexOf(emptyfieldpartmodel.model_name) === -1) {
            setemptyfieldpart([
              ...emptyfieldpart,
              emptyfieldpartmodel.model_name,
            ]);
          }
        } else {
          setemptyfieldpart((prev) =>
            prev.filter(
              (emptyfil) => emptyfil !== emptyfieldpartmodel.model_name
            )
          );
        }
      }
