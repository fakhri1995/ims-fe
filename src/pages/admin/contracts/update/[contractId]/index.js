...
function UpdateContract({
  ...
  const onChangeServicePriceItems = (e, id) => {
    var val = e.target.value;
    const idx = serviceItemTemp.serviceItemValues
      .map((item) => item.id_service_item)
      .indexOf(id);
    // console.log(idx,id)
            ...
            <>
              ...
              <div className="col-span-1 md:col-span-1 flex flex-col md:pl-2 py-2">
                <Select
                  ...
                  allowClear
                  onChange={(val) => {
                    onChangeServiceTermsofPaymentItems(
                      val,
                      item.id_service_item
                    );
                  }}
                >
  ...
  const onChangeRowSelection = (selectedRowKeys, selectedRows) => {
    ...
    const a3 = selectedRows.map((t1) => ({
      ...t1,
      ...dataServiceItems.find((t2) => t2.id_service_item === t1.key),
    }));
    a3.map((obj) => {
      if (obj.harga == null) {
        obj.harga = "";
  ...
  const onChangeCategory = (val) => {
    setIdCategory(val);
    const listServiceByCategory = contractInputData.data.service_items
      .map((doc, idx) => {
        return {
  ...
  const searchServiceItem = (e) => {
    ...
    const listServiceByCategory = contractInputData.data.service_items
      ...
      .filter((doc) => {
        if (idCategory != null) {
          return (
            doc.id_service_kategori == idCategory &&
            doc.nama.toLowerCase().includes(val)
          );
        } else {
  ...
  const populateListCompany = contractInputData.data.companies
    .filter((doc, idx) => doc.id !== 66)
    .map((doc, idx) => {
      return {
        value: doc.id,
  ...
  useEffect(() => {
    ...
    setSelectedRowKeys(
      contractData.data.service_item_kontraks.map((item, idx) => {
          ...
          >
            ...
            >
              <Sticky containerSelectorFocus="#formAgentsWrapper">
                <div className="flex justify-between p-4 border-gray-400 border-t border-b bg-white mb-8">
                  ...
                  <div className="flex space-x-2">
                    ...
                    <Button
                      disabled={
                        !(validation.harga && validation.id_terms_of_payment)
                      }
                      type="primary"
                      size="middle"
              ...
              <p className="text-red-600 float-right ml-auto">
                {validation.harga
                  ? ""
                  : "Harga pada item service harus terisi semua"}
              </p>
              <p className="text-red-600 float-right ml-auto">
                {validation.id_terms_of_payment
                  ? ""
                  : "Terms of Payment pada item service harus terisi semua"}
              </p>
                ...
                <div className=" col-span-1 md:col-span-1 flex flex-col">
                  ...
                  <div className="pb-4 md:mb-0 ">
                    <Form.Item
                      ...
                      initialValue={moment(
                        updatecontract.tanggal_selesai,
                        "YYYY-MM-DD"
                      )}
                    >
                      <DatePicker
                  ...
                  <div className="pb-4 md:mb-0 ">
                    ...
                    >
                      <Select
                        ...
                        allowClear
                      >
                        {contractInputData.data.contract_types.map(
                          (doc, index) => {
                            return (
              ...
              <div className="pb-4 md:mb-0">
                ...
                >
                  {" "}
                  <PlusSquareOutlined style={{ verticalAlign: "2px" }} /> Daftar
                  Service Item
                </a>
              </div>
              <div className={"w-full h-auto grid grid-cols-1 md:grid-cols-3"}>
              ...
              <Drawer
                ...
                onClose={() => {
                  setOpendrawer(false),
                    ...
                    setSelectedRowKeys(
                      selectedServiceItemTemp.map(
                        (item) => item.id_service_item
                      )
                    );
                }}
                destroyOnClose={true}
              ...
              >
                <div className={"flex flex-row justify-between"}>
                  <div className={"pb-6 flex"}>
                    ...
                    >
                      {contractInputData.data.service_categories.map(
                        (doc, index) => {
                          return (
                            <Option key={doc.id} value={doc.id}>
                              {doc.nama_kategori}
                    ...
                    <div className={"pl-2"}>
                      <Button className={""} onClick={clearFilterCategory}>
                        Clear Filter
                      </Button>
                    </div>
                  </div>
