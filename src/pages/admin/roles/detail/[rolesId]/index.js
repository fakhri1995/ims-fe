    ...
    },
    {
      ...
      render(t, r, i) {
        return (
          <>
            {r.status ? (
              <div className="rounded-md w-2/5 mx-auto h-auto px-1 text-center justify-center py-1 bg-blue-100 border border-blue-200 text-blue-600">
                Aktif
              </div>
            ) : (
              <div className="rounded-md w-2/5 mx-auto h-auto px-1 text-center justify-center py-1 bg-red-100 border border-red-200 text-red-600">
                Non-aktif
              </div>
            )}
          </>
...
const FiturSide = ({ initProps, rolesid }) => {
  ...
  const col = [
    ...
    },
    {
      ...
      render(t, r, i) {
        var strmodule =
          r.list_module.length > 0 ? r.list_module.join(",") : "-";
        return <>{strmodule}</>;
      },
      // render(text, record, index) {
      //     return {
    ...
    },
    {
      ...
      key: "description",
      render(t, r, i) {
        return <div className="text-xs">{r.description}</div>;
      },
      // render(text, record, index) {
      //     return {
      //         props: {
