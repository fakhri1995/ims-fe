...
const AssetUpdate = ({ sidemenu, dataProfile, initProps, assettypeid }) => {
  ...
  const [currentnondropdowntrigger, setcurrentnondropdowntrigger] =
    useState(false);
  ...
  //useEffect
  useEffect(() => {
    if (!isAllowedToUpdateAsset) {
      permissionWarningNotification("Memperbarui", "Tipe Asset");
    }
  }, [isAllowedToUpdateAsset]);
  ...
  useEffect(() => {
    if (cdidx !== -1) {
      ...
      setcurrentdropdown2((prev) =>
        prev.filter((doc11, idx11) => idx11 !== cdidx)
      );
    }
  }, [cdtrigger]);
  ...
  return (
    ...
    >
      ...
      >
            ...
            <div className=" col-span-4 flex justify-between p-2 pt-4 border-t-2 border-b-2 bg-white">
              ...
              <div className="flex space-x-2">
                ...
                <Button
                  ...
                  disabled={disabledsimpan || !isAllowedToUpdateAsset}
                >
                  Simpan
                </Button>
              </div>
        ...
        <AccessControl hasPermission={ASSET_UPDATE}>
          ...
          <div className=" col-span-1 md:col-span-4 px-5 flex flex-col">
                      ...
                      <Form layout="vertical" initialValues={fielddata[idx]}>
                        ...
                        <div className="flex mt-4 justify-end">
                          <Popconfirm
                            ...
                            onConfirm={() => {
                              if (
                                displaydata.asset_columns.some(
                                  (docc) => docc.id === doc.id
                                )
                              ) {
                                setdeletefielddata([
                          ...
                          <Button
                            ...
                            onClick={() => {
                              if (
                                displaydata.asset_columns
                                  .map((docc) => docc.id)
                                  .includes(doc.id) === false
                              ) {
                                if (
                                  updatedata.add_columns
                                    .map((docname) => docname.name)
                                    .indexOf(doc.name) === -1
                                ) {
                                  if (currentfield.data_type === "dropdown") {
                                    const newwdd = {
                                  ...
                                  setnewfielddata((prev) => {
                                    var temp = prev;
                                    const idxnewfield = updatedata.add_columns
                                      .map((docname) => docname.name)
                                      .indexOf(doc.name);
                                    if (currentfield.data_type === "dropdown") {
                                      temp[idxnewfield] = {
                                ...
                                const idxupdatefield = updatefielddata
                                  .map((docv) => docv.id)
                                  .indexOf(doc.id);
                                if (idxupdatefield === -1) {
                                  if (currentfield.data_type === "dropdown") {
                              ...
                              if (addedfield[idx] === false) {
                                ...
                                typeof (addedfield[idx] === "undefined")
                              ) {
            ...
            <div className="w-full flex justify-center mt-5">
              ...
              >
                + Tambah Spesifikasi Asset Type
              </Button>
            </div>
          </div>

          <Modal
            title={<strong>Konfirmasi Ubah Induk Asset Type</strong>}
          ...
          >
            <div className="flex flex-col p-5">
              ...
              >
                ...
                <div className="mb-2">
                  <h5 className=" text-xs font-semibold">
                    Apakah anda ingin membawa seluruh Child dari asset type "
                    {displaydata.name}"?
                  </h5>
                  <Radio.Group
                  ...
                  >
                    <div className="flex flex-col">
                      ...
                      <Radio value={false}>
                        Tidak, saya ingin seluruh child dari asset type "
                        {displaydata.name}" dihapus
                      </Radio>
                    </div>
                  </Radio.Group>
                </div>
