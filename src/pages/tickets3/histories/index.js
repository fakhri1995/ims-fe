  ...
  const column = [
    {
      ...
      render: (text, record, index) => {
        return {
          children: (
            <>
              {record.type.id === 1 && `#${record.type.code} - `}
              {record.ticketable.id}
            </>
          ),
        };
      },
      ...
      fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/${
          dataProfile.data.role === 1
            ? "getClosedTickets"
            : "getClientClosedTickets"
        }?ticket_id=${namasearchact ? namavalue : ""}`,
        {
          method: `GET`,
          headers: {
        ...
        .then((res) => res.json())
        .then((res2) => {
          res2.data.tickets.data.length === 0
            ? setdisplaydata([])
    ...
    >
      <div className=" w-full grid grid-cols-1 md:grid-cols-4 border-t border-b bg-white mb-5 px-4 py-5">
        <div className=" col-span-1 md:col-span-3 flex items-center mb-2 md:mb-0">
          <div className="font-bold text-2xl w-auto mr-14">
            History Tickets Closed
          </div>
        </div>
