        ...
        onChange: (page, pageSize) => {
          ...
          fetch(
            `${
              ...
              sorting.sort_by
            }&sort_type=${sorting.sort_type}&is_posted=${
        ...
        if (extra.action === "sort") {
          if (sorter.column) {
            ...
            fetch(
              `${
                process.env.NEXT_PUBLIC_BACKEND_URL
              }/getEmployeePayslips?employee_id=${employeeId}&is_posted=${
                selectedPayslipStatusId ? selectedPayslipStatusId - 1 : null
              }&sort_by=${sorter.column.dataIndex}&sort_type=${
                sorter.order === "ascend" ? "asc" : "desc"
              }&keyword=${searching}&page=${pagination.current}&rows=${
                pagination.pageSize
              }`,
            ...
            fetch(
              `${
                ...
                selectedPayslipStatusId ? selectedPayslipStatusId - 1 : null
              }&sort_by=&sort_type=&keyword=${searching}&page=${
                pagination.current
              }&rows=${pagination.pageSize}`,
              {
                method: `GET`,
