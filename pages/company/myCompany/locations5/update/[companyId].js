                            ...
                            <div className="flex items-center mx-2">
                                {
                                    editable ?
                                        <>
                                            ...
                                            <Button type="default" onClick={() => { seteditable(false) }} size="middle" style={{ marginRight: `1rem` }}>Cancel</Button>
                                            {/* </Link> */}
...
export async function getServerSideProps({ req, res, params }) {
    ...
    if (![151, 153, 154].every((curr) => dataProfile.data.registered_feature.includes(curr))) {
        res.writeHead(302, { Location: '/company/company/mig' })
        res.end()
    }
    ...
    if(!dataBranchDetail.success){
        return{
            redirect:{
                permanent: false,
                destination: '/company/company/mig'
            }
        }
    }
