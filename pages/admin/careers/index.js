                    ...
                    children:
                        <>
                            <a href={record.register_link} target="_blank">
                                <h1 className=" text-blue-400 hover:text-blue-800 text-xs">{record.register_link} <SelectOutlined /></h1>
                            </a>
                        </>
                }
            }
        ...
        <Layout tok={initProps} dataProfile={dataProfile} pathArr={pathArr} sidemenu={sidemenu} st={st}>
            <div className="w-full grid grid-cols-5 border-t border-opacity-30 border-gray-500 bg-white">
                ...
                <div className="col-span-5 p-0 md:p-5 flex flex-col">
                    <Table columns={columnsFeature} dataSource={dataCareersMap} pagination={{ pageSize: 8 }} scroll={{ x: 300 }}></Table>
                </div>
            </div>
            <Drawer title={`Add A New Career`} maskClosable={false} visible={drawcreate} onClose={() => { setdrawcreate(false) }} width={380} destroyOnClose={true}>
                <div className="flex flex-col">
                    <Form layout="vertical" initialValues={datacreate} onFinish={handleCreate}>
                        <Form.Item label="Position Name" name="position_name" rules={[
                            {
                                required: true,
                                message: 'Position Name wajib diisi',
                            },
                        ]}>
                            <Input defaultValue={datacreate.position_name} onChange={(e) => { setdatacreate({ ...datacreate, position_name: e.target.value }) }} />
                        </Form.Item>
                        <Form.Item label="Description" name="job_description" rules={[
                            {
                                required: true,
                                message: 'Description Job wajib diisi',
                            },
                        ]}>
                            <Input.TextArea defaultValue={datacreate.job_description} onChange={(e) => { setdatacreate({ ...datacreate, job_description: e.target.value }) }} />
                        </Form.Item>
                        <Form.Item label="Category" name="job_category" rules={[
                            {
                                required: true,
                                message: 'Category wajib diisi',
                            },
                        ]}>
                            <Input defaultValue={datacreate.job_category} onChange={(e) => { setdatacreate({ ...datacreate, job_category: e.target.value }) }} />
                        </Form.Item>
                        <Form.Item label="Register Link" name="register_link" rules={[
                            {
                                required: true,
                                message: 'Register Link wajib diisi',
                            },
                        ]}>
                            <Input defaultValue={datacreate.register_link} onChange={(e) => { setdatacreate({ ...datacreate, register_link: e.target.value }) }} />
                        </Form.Item>
                        <div className="flex justify-end">
                            <Button type="default" onClick={() => { setdrawcreate(false) }} style={{ marginRight: `1rem` }}>Cancel</Button>
                            <Button htmlType="submit" type="primary" loading={loadingcreate}>Save</Button>
                        </div>
                    </Form>
                </div>
            </Drawer>
            <Drawer title={`Edit Career`} maskClosable={false} visible={drawedit} onClose={() => { setdrawedit(false); }} width={380} destroyOnClose={true}>
                <div className="flex flex-col">
                    <Form layout="vertical" initialValues={dataedit} onFinish={handleEdit}>
                        <Form.Item label="Position Name" name="position_name" rules={[
                            ...
                            <Input defaultValue={dataedit.position_name} onChange={(e) => { setdataedit({ ...dataedit, position_name: e.target.value }) }} />
                        </Form.Item>
                        <Form.Item label="Description" name="job_description" rules={[
                            {
                                required: true,
                            ...
                            <Input.TextArea defaultValue={dataedit.job_description} onChange={(e) => { setdataedit({ ...dataedit, job_description: e.target.value }) }} />
                        </Form.Item>
                        <Form.Item label="Category" name="job_category" rules={[
                            {
                                required: true,
                            ...
                            <Input defaultValue={dataedit.job_category} onChange={(e) => { setdataedit({ ...dataedit, job_category: e.target.value }) }} />
                        </Form.Item>
                        <Form.Item label="Register Link" name="register_link" rules={[
                            {
                                required: true,
                            ...
                            <Input defaultValue={dataedit.register_link} onChange={(e) => { setdataedit({ ...dataedit, register_link: e.target.value }) }} />
                        </Form.Item>
                        <div className="flex justify-end">
                            <Button type="default" onClick={() => { setdrawedit(false) }} style={{ marginRight: `1rem` }}>Cancel</Button>
                            <Button htmlType="submit" type="primary" loading={loadingedit}>Save</Button>
                        </div>
                    </Form>
                </div>
            </Drawer>
