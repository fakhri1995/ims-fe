...
import { Button, DatePicker, Select, Checkbox, Divider, Empty } from 'antd'
...
const TicketExporting = ({ initProps, dataProfile, sidemenu }) => {
    ...
    const [defaultcoreopt, setdefaultcoreopt] = useState([1, 2, 3, 4, 5, 6, 7, 8]);
    ...
    const onCheckAllChangecore = e => {
        ...
        var arr = [1, 1, 1, 1, 1, 1, 1, 1, 1], arr2 = [0, 0, 0, 0, 0, 0, 0, 0, 0]
    ...
    const onCheckAllChangesec = e => {
        ...
        var arr = [1, 1, 1, 1, 1, 1, 1, 1], arr2 = [0, 0, 0, 0, 0, 0, 0, 0]
    ...
    const handleExport = () => {
        ...
        fetch(`https://boiling-thicket-46501.herokuapp.com/ticketsExport?group=${attr.group === null ? "" : attr.group}&engineer=${attr.engineer === null ? "" : attr.engineer}&type=${attr.type === null ? null : attr.type}&from=${attr.from}&to=${attr.to}&core_attributes=[${attr.core_attributes}]&secondary_attributes=[${attr.secondary_attributes}]${closed === "1" ? `&is_history=1` : `&is_history=0`}`, {
            ...
            .then(res => res.blob())
            .then(res2 => {
                var newBlob = new Blob([res2], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" })
        ...
        <Layout dataProfile={dataProfile} sidemenu={sidemenu} tok={initProps} st={st} pathArr={pathArr}>
            <div className=" w-full grid grid-cols-1 md:grid-cols-4 border-gray-400 bg-white mb-5 px-4 py-5">
                ...
                <div className=" col-span-1 md:col-span-1 flex md:justify-end items-center">
                    ...
                    <Button size="large" type="primary" style={{ marginRight: `1rem` }} onClick={handleExport}>
                        Unduh
                    </Button>
                    {/* </a> */}
                </div>
            ...
            <div className="h-auto w-full grid grid-cols-1 md:grid-cols-5 mb-5 bg-white rounded-md">
                <div className="md:col-span-5 col-span-1 flex flex-col py-3">
                    ...
                    <div className="border-2 shadow-md rounded-md flex flex-col py-5 px-10 mb-9 w-9/12">
                        <div className="flex flex-col mb-5">
                            ...
                            <DatePicker.RangePicker onChange={(dates, datestrings) => {
                                if (datestrings[0] === '' && datestrings[1] === '') {
                                    setattr({ ...attr, from: "", to: "" })
                                }
                                else {
                                    setattr({ ...attr, from: moment(datestrings[0]).locale('id').format('YYYY-MM-DD'), to: moment(datestrings[1]).locale('id').format('YYYY-MM-DD') })
                                }
                            }}></DatePicker.RangePicker>
                        </div>
                        <div className="flex flex-col mb-5">
                            <p className="mb-2 font-semibold text-sm">Group</p>
                            <Select loading={praloading} onChange={(value) => {
                                setattr({
                                    ...attr,
                                    group: Number(value)
                                })
                ...
                <div className="md:col-span-5 col-span-1 flex flex-col py-3">
                    ...
                    <Select allowClear placeholder="Pilih Tipe Ticket" style={{ marginBottom: `1rem`, width: `25%` }} onChange={(value) => {
                        console.log(value)
                        setattr({ ...attr, type: typeof (value) === 'undefined' ? null : value })
                    ...
                    <div className="flex flex-col mb-4">
                        <div className="font-semibold w-9/12 border-b mb-3">Atribut Utama</div>
                        <Checkbox style={{ marginBottom: `0.5rem` }} indeterminate={indeterminatecore} onChange={onCheckAllChangecore} checked={checkAllcore}>
                            <strong>Pilih Semua</strong>
                        </Checkbox>
                        <CheckboxGroup value={defaultcoreopt} onChange={onChangecore}>
                            <div className=" w-9/12 grid grid-cols-1 md:grid-cols-3 mr-5">
                                <Checkbox value={coreopt[0]} style={{ marginBottom: `0.5rem`, marginLeft: `0rem` }}>Nama Pembuat</Checkbox>
                                <Checkbox value={coreopt[1]} style={{ marginBottom: `0.5rem`, marginLeft: `0rem` }}>Lokasi Pembuat</Checkbox>
                                <Checkbox value={coreopt[2]} style={{ marginBottom: `0.5rem`, marginLeft: `0rem` }}>Data Raised Ticket</Checkbox>
                                <Checkbox value={coreopt[3]} style={{ marginBottom: `0.5rem`, marginLeft: `0rem` }}>Data Closed Ticket</Checkbox>
                                <Checkbox value={coreopt[4]} style={{ marginBottom: `0.5rem`, marginLeft: `0rem` }}>Ticket Number</Checkbox>
                                <Checkbox value={coreopt[5]} style={{ marginBottom: `0.5rem`, marginLeft: `0rem` }}>Ticket Type</Checkbox>
                                <Checkbox value={coreopt[6]} style={{ marginBottom: `0.5rem`, marginLeft: `0rem` }}>Status Ticket</Checkbox>
                                <Checkbox value={coreopt[7]} style={{ marginBottom: `0.5rem`, marginLeft: `0rem` }}>Assign To</Checkbox>
                            </div>
                        </CheckboxGroup>
                    </div>
                    <div className="flex flex-col mb-4 w-9/12">
                        <div className="font-semibold w-full border-b mb-3">Atribut Tambahan</div>
                        {
                            attr.type === null ?
                                <>
                                    <Empty image={Empty.PRESENTED_IMAGE_DEFAULT} description="Jika Tipe Ticket tidak dipilih, akan menampilkan semua field" />
                                </>
                                :
                                <>
                                    <Checkbox style={{ marginBottom: `0.5rem` }} indeterminate={indeterminatesec} onChange={onCheckAllChangesec} checked={checkAllsec}>
                                        <strong>Pilih Semua</strong>
                                    </Checkbox>
                                    <CheckboxGroup value={defaultsecopt} onChange={onChangesec}>
                                        <div className=" w-9/12 grid grid-cols-1 md:grid-cols-3 mr-5">
                                            <Checkbox value={secopt[0]} style={{ marginBottom: `0.5rem`, marginLeft: `0rem` }}>Jenis Produk</Checkbox>
                                            <Checkbox value={secopt[1]} style={{ marginBottom: `0.5rem`, marginLeft: `0rem` }}>ID Produk</Checkbox>
                                            <Checkbox value={secopt[2]} style={{ marginBottom: `0.5rem`, marginLeft: `0rem` }}>Nama PIC</Checkbox>
                                            <Checkbox value={secopt[3]} style={{ marginBottom: `0.5rem`, marginLeft: `0rem` }}>Kontak PIC</Checkbox>
                                            <Checkbox value={secopt[4]} style={{ marginBottom: `0.5rem`, marginLeft: `0rem` }}>Problem</Checkbox>
                                            <Checkbox value={secopt[5]} style={{ marginBottom: `0.5rem`, marginLeft: `0rem` }}>Lokasi Problem</Checkbox>
                                            <Checkbox value={secopt[6]} style={{ marginBottom: `0.5rem`, marginLeft: `0rem` }}>Waktu Kejadian</Checkbox>
                                        </div>
                                    </CheckboxGroup>
                                </>
                        }
