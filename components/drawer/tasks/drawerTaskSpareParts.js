...
const DrawerTaskSpareParts = ({ title, visible, onvisible, onClose, buttonOkText, disabled, initProps, idtask, selectedforin, setselectedforin, selectedforout, setselectedforout, prevpath }) => {
    //useState
    ...
    return (
        <DrawerCore
            ...
            onClick={() => { console.log(datapayload, selectedforin); handleSendSpareParts() }}
                                        ...
                                        <div className=' mb-2 flex'>
                                            <Select
                                                ...
                                                onChange={(value, option) => {
                                                    setdatapayload({
                                                        ...datapayload,
                                                        add_in_inventories: [...datapayload.add_in_inventories, { inventory_id: value, connect_id: 0 }]
                                                    });
                                                ...
                                                notFoundContent={fetchingstate ? <Spin size="small" /> : null}
                                                onSearch={(value) => {
                                            ...
                                            >
                                                {
                                                    dataselectforin.map((doc, idx) => (
                                                        <Select.Option key={idx} value={doc.id} migid={doc.mig_id} modelname={doc.model_name} assetname={doc.asset_name}>{doc.mig_id} - {doc.model_name} - {doc.asset_name}</Select.Option>
                                                    ))
                                                }
                                            </Select>
                                        </div>
                                        ...
                                        <div className=' mb-2 flex'>
                                            <TreeSelect
                                                ...
                                                onChange={(value, label, extra) => {
                                                    setselectedforout([...selectedforout, { value: extra.allCheckedNodes[0].node.props.value, migid: extra.allCheckedNodes[0].node.props.mig_id, modelname: extra.allCheckedNodes[0].node.props.model_inventory?.name, assetname: extra.allCheckedNodes[0].node.props.model_inventory?.asset?.name }]);
                                                    setdatapayload(prev => ({
                                                        ...prev,
