...
const ButtonSys = ({ size, type, children, color, submit, onClick, form }) => {
    if (type === 'primary') {
        ...
        return(
            <button form={form} onClick={onClick} type={submit ? "submit" : "button"} className={`btn btn-outline border-none ${size === "large" ? "" : "btn-sm"} ${typeof(color) === 'undefined' ? "text-primary100" : (color === "danger" && "text-state1")} hover:${typeof(color) === 'undefined' ? "text-primary75" : (color === "danger" && "text-state1")} px-6`}>
                {children}
            </button>
        )
    }
